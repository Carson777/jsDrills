...
var getFullNames = function(objectArray){
    ...
    var tempString = ""
    var finalArray = []
    for(var i = 0; i < objectArray.length; i = i +1){
    var tempFirst = objectArray[i].first
    var tempLast = objectArray[i].last
    var tempString = tempFirst + " " + tempLast
...
function generateDog(name) {
    ...
    for(var i = 0; i < tempArray.length; i = i +1){
        ...
        rString = "r" + tempString.substring(1, tempString.length) + " "
        finalString = finalString + rString
    }
...
var pluck = function(inputArray, inputProperty){
    var tempString = ""
    var finalArray = []
    for(var i = 0; i < inputArray.length; i = i + 1){
        tempString = inputArray[i][inputProperty]
        finalArray.push(tempString)
    }
    return finalArray
}
...
var getCounts = function(inputString){
    ...
    //creates an array with all words from input string in order
    var words = []
    var tempObject = {}
    var currentWord = ""
    for(var i = 0; i < splitArray.length; i = i + 1){
            var tempString = splitArray[i]
            for(var x = 0; x < words.length; x = x + 1){
                if(!(tempString === words[x])){
                    words.push(tempString)
                    //this should fill the words array with a list of each word
                }
            }
            for(var c = 0; c < words.length; c = c + 1){
                tempObject[words[c]] = 0
                //this SHOULD create a new property for tempObject and assigned its value to 0
            }
            for(var v = 0; v < words.length; v = v + 1){
                currentWord = words[v]
                    for( var b = 0; b < splitArray; b = b + 1){
                        if(currentWord = splitArray[b]){
                            tempObject[currentWord] = tempObject[currentWord] + 1
                            //Every time currentWord is found inside of splitArray, it should increase its corrosponding property by 1
                        }
                    }
            }
    }
